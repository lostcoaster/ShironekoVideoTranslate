...
void FinalizerCreater::generateCommand(QString filename) {
	...
	fs << "%ffmpegPath% -ss %audioStartSecond% -i %sourceFile% -map 0:a -acodec aac ~sound.aac\r\n";
	fs << "%ffmpegPath% -i ~sound.aac -i %destFile% -map 0:a -map 1:v -c copy -shortest ~temp_video.avi\r\n";
	fs << "del ~sound.aac \r\n";
	fs << "move /Y ~temp_video.avi %destFile%\r\n";
